...
num1Input.addEventListener("input", () => {   // forma tradicional de validar
...
num2Input.addEventListener("input", () => {   // usando operadores ternarios
  const valor = parseFloat(num2Input.value);
  document.getElementById("error-num2").textContent = isNaN(valor) ? "⚠️ Número inválido" : "";
});

operacionSelect.addEventListener("change", () => { // usando operadores ternarios
...
function validarFormulario() {
  ...
  if (esValido) {    // si cumple todas las condiciones (true), se habilita y cambia de color
